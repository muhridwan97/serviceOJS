...
class fileController{
    ...
    public function uploadArsipEdit($request, $response, $args) {
        $body=$request->getParsedBody();
        $submission_id=$body['submission_id'];
        $editor_id=$body['editor_id'];
        $uploadedFiles = $request->getUploadedFiles();
        ...
        $userFiles = new \model\fileModel();
        $dataUserFiles = $userFiles->setFileArsipEdit($data);
        $file_id=$dataUserFiles[file_id][0]->file_id;
         $revision=$dataUserFiles[revision];
        // print_r($file_id);
        // print_r($revision);
        $path="C:\\xampp\\htdocs\\jurnal\\dataJurnal\\journals\\2\\articles\\$submission_id\\submission\productionReady";
        ...
        // print_r($uploadedFiles['fileArsip']);
         ...
         if ($uploadedFile->getError() === UPLOAD_ERR_OK) {
              $extension = pathinfo($uploadedFile->getClientFilename(), PATHINFO_EXTENSION);
              $namaBaru="".$submission_id."-"."13"."-".$file_id."-".$revision."-"."11"."-".$date;
              $filename = sprintf('%s.%0.8s', $namaBaru, $extension);
            //print_r($filename);
             $uploadedFile->moveTo($directory . DIRECTORY_SEPARATOR . $filename);
             return $response->write('uploaded ' . $filename . 'berhasil <br/>');
    ...
    public function uploadGalleyEdit($request, $response, $args) {
        $body=$request->getParsedBody();
        $submission_id=$body['submission_id'];
        $editor_id=$body['editor_id'];
        $uploadedFiles = $request->getUploadedFiles();
        ...
        $userFiles = new \model\fileModel();
        $dataUserFiles = $userFiles->setFileGalleyEdit($data);
         $file_id=$dataUserFiles[file_id][0]->file_id;
         $revision=$dataUserFiles[revision];
        print_r($file_id);
        print_r($revision);
        print_r($dataUserFiles);
        $path="C:\\xampp\\htdocs\\jurnal\\dataJurnal\\journals\\2\\articles\\$submission_id\\submission\proof";
        ...
        // print_r($uploadedFiles['fileArsip']);
         ...
         if ($uploadedFile->getError() === UPLOAD_ERR_OK) {
              $extension = pathinfo($uploadedFile->getClientFilename(), PATHINFO_EXTENSION);
              $namaBaru="".$submission_id."-"."13"."-".$file_id."-".$revision."-"."10"."-".$date;
              $filename = sprintf('%s.%0.8s', $namaBaru, $extension);
        //print_r($filename);
             $uploadedFile->moveTo($directory . DIRECTORY_SEPARATOR . $filename);
             return $response->write('uploaded ' . $filename . 'berhasil <br/>');
